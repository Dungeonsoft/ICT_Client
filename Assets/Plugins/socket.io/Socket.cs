...
namespace socket.io {
    ...
    public class Socket : MonoBehaviour {
        ...
        public static Socket Connect(string url) {
            ...
            SM.name = "(singleton) socket.io.SocketManager";
            SM.AddComponent<SocketManager>();

            //SocketManager.Instance.Connect(socket);
            socket.transform.SetParent(SM.transform, false);
            socket.Url = new Uri(url);

            SM.GetComponent<SocketManager>().Connect(socket);
            //SocketManager.Instance.Connect(socket);
            return socket;
        }
